...
# НЕ ПРИНАДЛЕЖИТ КЛАССУ
    def OnLabelExecute(s):
        try:
            debug.SaveString("OnLabelExecute: "+s)
            if s[0]!="_":
    ...
    # Каждый раз при переходе по метке происходит проверка - не имя ли ивента метка? Если да, переменной event присваивается значение, согласно текущей метке
                if this(s)!=None:
                    debug.SaveString("this(s): "+s)
    # Если перешли на метку, которая есть ивент, автоматически запускается увеличение счетчика стартов. 
    # Случаи, когда ивент завершается (и требует увеличения счетчика финишей) обрабатываются в коде самого ивента                
                    this().LabelExecute()
